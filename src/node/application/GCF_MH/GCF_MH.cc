 ...
 */
void GCF_MH::fromNetworkLayer ( ApplicationPacket * genericPacket, const char *source, double rssi, double lqi )
{
...
void GCF_MH::timerFiredCallback ( int index )
{
	switch ( index ) {
	...
	case FIND_PATH: {
		...
		for ( i = 0; i < sensorsReported - 1; i++ ) {
			...
			for ( j = 0; j < numberOfSensors; j++ ) {
				if ( ( Info[0][j].cost < minCost ) && ( Info[j][j].used == false ) ) {
					minCost = Info[0][j].cost;
					minNode = j;
				}
			}
			if ( Info[currentNode][minNode].cost <= MaxTransmissionCost ) {
...
void GCF_MH::handleSensorReading ( SensorReadingMessage * sensorMsg )
{
...
void GCF_MH::DijkstraDistance ( int source, vector<double> &MinVect, vector<int> &PrevVect )
{
...
void GCF_MH::DijkstraPower ( int source, vector<double> &MinVect, vector<int> &PrevVect )
{
	set<pair<double, int> > vertex_queue;
