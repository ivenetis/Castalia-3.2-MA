...
Define_Module ( IEMA_MH );

/******************************************************************************/

void IEMA_MH::startup()
{
 ...
 */
void IEMA_MH::fromNetworkLayer ( ApplicationPacket * genericPacket, const char *source, double rssi, double lqi )
{
...
void IEMA_MH::timerFiredCallback ( int index )
{
	switch ( index ) {
	...
	case FIND_PATH: {
		...
		int	i, j, k, l, m, v, currentNode, minNode, nodesInPath, prevNodesInPath, intermediateNodes, searchNode, nextNode;
		 ...
		 */
		iterations = sensorsReported / 5;	// 20% of nodes in path.
		for ( m = 0; ( m < iterations ) && ( m < sensorsReported - 1 ); m++ ) {
			totalMinCost = INFINITY;
			for ( i = 1; i < numberOfSensors; i++ ) {
				if ( ( Info[i][i].nodeID != NO_ID ) && ( Info[i][i].used == false ) ) {
					searchNode = i;
					minNode    = i;	// Handles case where only one sensor has reported its position.
#if defined(LOG_ENABLED)
					...
					for ( j = 0; j < sensorsReported - m - 2; j++ ) {
						minCost = INFINITY;
						for ( k = 0; k < numberOfSensors; k++ ) {
							if ( ( Info[searchNode][k].cost < minCost ) && ( usedVec[k] == false ) ) {
								minCost = Info[searchNode][k].cost;
								minNode = k;
							}
						}
#if defined(LOG_ENABLED)
...
#if defined(LOG_ENABLED)
					trace() << "  Total cost for second node " << i << " is " << totalCost;
#endif
			...
			/*
			 * Put the next node into the path, which is the node that gives the least total cost for the mobile agent.
			 */
		 ...
		 */
		for ( i = 0; i < sensorsReported - m - 1; i++ ) {
			minCost = INFINITY;
			for ( j = 0; j < numberOfSensors; j++ ) {
				if ( ( Info[currentNode][j].cost < minCost ) && ( Info[j][j].used == false ) ) {
...
void IEMA_MH::handleSensorReading ( SensorReadingMessage * sensorMsg )
{
...
void IEMA_MH::DijkstraDistance ( int source, vector<double> &MinVect, vector<int> &PrevVect )
{
...
void IEMA_MH::DijkstraPower ( int source, vector<double> &MinVect, vector<int> &PrevVect )
{
	set<pair<double, int> > vertex_queue;
