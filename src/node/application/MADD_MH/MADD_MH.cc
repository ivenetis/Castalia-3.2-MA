...
Define_Module ( MADD_MH );

/******************************************************************************/

void MADD_MH::startup()
{
 ...
 */
void MADD_MH::fromNetworkLayer ( ApplicationPacket * genericPacket, const char *source, double rssi, double lqi )
{
...
void MADD_MH::timerFiredCallback ( int index )
{
	switch ( index ) {
	...
	case FIND_PATH: {
		double	minCost, maxDistance, minDistance, travelDistance;
		int	i, j, k, l, v, currentNode, minNode, nodesInPath, prevNodesInPath, intermediateNodes, nextNode, lastNode;

		/*
		 * Find the nodes that have the largest and smallest distance from the sink (node 0).
		 */
		maxDistance = 0.0;
		minDistance = INFINITY;
		for ( i = 1; i < numberOfSensors; i++ ) {
			if ( ( Info[i][i].nodeID != NO_ID ) && ( Info[0][i].distance > maxDistance ) ) {
				maxDistance = Info[0][i].distance;
				nextNode = i;
			}
			if ( ( Info[i][i].nodeID != NO_ID ) && ( Info[0][i].distance < minDistance ) ) {
				minDistance = Info[0][i].distance;
				lastNode = i;
			}
		}

		trace() << "Next node found is " << nextNode;
		trace() << "Last node found is " << lastNode;
		...
		/*
		 * Put the first node into the path, which is the node that has the largest distance from the sink (node 0).
		 */
		 ...
		 */
		for ( i = 0; i < sensorsReported - 3; i++ ) {
			minCost = INFINITY;
			for ( j = 0; j < numberOfSensors; j++ ) {
				if ( ( Info[currentNode][j].cost < minCost ) && ( Info[j][j].used == false ) && ( j != lastNode ) ) {
					minCost = Info[currentNode][j].cost;
					minNode = j;
				}
			}
		...
		/*
		 * Put the last node into the path, which is the node that has the smallest distance from the sink (node 0).
		 */
		nextNode = lastNode;
		if ( Info[currentNode][nextNode].cost <= MaxTransmissionCost ) {
			Path[nodesInPath].nodeID   = nextNode;
...
void MADD_MH::handleSensorReading ( SensorReadingMessage * sensorMsg )
{
...
void MADD_MH::DijkstraDistance ( int source, vector<double> &MinVect, vector<int> &PrevVect )
{
...
void MADD_MH::DijkstraPower ( int source, vector<double> &MinVect, vector<int> &PrevVect )
{
	set<pair<double, int> > vertex_queue;
