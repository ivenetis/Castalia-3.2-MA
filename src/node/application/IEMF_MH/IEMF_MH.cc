...
Define_Module ( IEMF_MH );

/******************************************************************************/

void IEMF_MH::startup()
{
 ...
 */
void IEMF_MH::fromNetworkLayer ( ApplicationPacket * genericPacket, const char *source, double rssi, double lqi )
{
...
void IEMF_MH::timerFiredCallback ( int index )
{
	switch ( index ) {
	...
	case FIND_PATH: {
		...
		int	i, j, k, l, v, currentNode, minNode, nodesInPath, prevNodesInPath, intermediateNodes, nextNode;
...
#if defined(LOG_ENABLED)
				trace() << "Searching for node " << currentNode;
				trace() << "  Cost from 0 to " << currentNode << ": " << Info[0][currentNode].cost;
#endif
				totalCost = Info[0][currentNode].cost;
				for ( j = 0; j < numberOfSensors; j++ ) {
					Info[j][j].used = false;
				}
				Info[0][0].used = true;
				Info[currentNode][currentNode].used = true;
				for ( j = 0; j < sensorsReported - 2; j++ ) {
					minCost = INFINITY;
					for ( k = 0; k < numberOfSensors; k++ ) {
						if ( ( Info[currentNode][k].cost < minCost ) && ( Info[k][k].used == false ) ) {
							minCost = Info[currentNode][k].cost;
							minNode = k;
						}
					}
#if defined(LOG_ENABLED)
...
void IEMF_MH::handleSensorReading ( SensorReadingMessage * sensorMsg )
{
...
void IEMF_MH::DijkstraDistance ( int source, vector<double> &MinVect, vector<int> &PrevVect )
{
...
void IEMF_MH::DijkstraPower ( int source, vector<double> &MinVect, vector<int> &PrevVect )
{
	set<pair<double, int> > vertex_queue;
